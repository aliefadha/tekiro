...
export default function GalleryPageClient() {
  const [activeView, setActiveView] = useState<"gallery" | "instagram">(
    "gallery",
  );
  const { data: galleryResponse, isLoading: isGalleryLoading } = useGallery();
  const { data: instagramResponse, isLoading: isInstagramLoading } =
    useInstagram();
  ...
  const isLoading =
    activeView === "gallery" ? isGalleryLoading : isInstagramLoading;

  return (
    <>
      <div className="mx-auto px-5 lg:px-24 py-5 lg:py-10 flex flex-col gap-5">
        <h1 className={`${montserrat.className} font-bold text-3xl uppercase`}>
          Gallery
        </h1>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`p-4 text-center font-bold transition-colors duration-200 cursor-pointer ${
              activeView === "gallery"
