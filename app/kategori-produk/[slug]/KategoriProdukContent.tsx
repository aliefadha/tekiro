...
export default function KategoriProdukContent({ slug }: { slug: string }) {
  ...
  const categories: Category[] = (
    (categoriesResponse?.data || []) as Array<{ id: string; name: string }>
  ).map((item) => ({
    id: item.id,
    label: item.name,
  ...
  const { data: productsResponse, isLoading: isProductsLoading } =
    useProductsByCategory(currentCategory?.id || "");
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row max-w-[1400px] h-full gap-5 px-5">
        ...
        {isProductsLoading ? (
          ...
          <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 auto-rows-fr w-full">
            {products.length > 0 ? (
