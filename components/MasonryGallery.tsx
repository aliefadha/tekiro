...
export default function MasonryGallery({
  ...
  if (!images || images.length === 0) {
    return (
      <div
        className={`w-full h-96 bg-gray-200 rounded-lg flex items-center justify-center ${className}`}
      >
  ...
  const rows: {
    left: { image: string; index: number };
    right?: { image: string; index: number };
  }[] = [];
          ...
          {rows.map((row, rowIndex) => {
            ...
            return (
              <div key={rowIndex} className="flex gap-4 h-[300px]">
                ...
                >
                  <Image
                    src={getImageUrl(row.left.image)}
                ...
                {row.right && (
                  ...
                  >
                    <Image
                      src={getImageUrl(row.right.image)}
