  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-[400px] md:h-[550px] relative items-center justify-center">
      ...
      <div className="w-full md:w-2/5 bg-black z-10 md:h-full flex flex-col justify-center p-4 md:p-0">
        <div
          ref={greenBgRef}
          className="bg-[#85E408] text-center w-full max-w-none p-6 md:px-12 md:py-24 flex flex-col gap-5 md:-ms-60"
        >
          <h2
...
export default function CordlessCarousel({ products }: CordlessCarouselProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        {products.map((product: CordlessProduct, index: number) => (
          <SwiperSlide key={product.id}>
            <ProductSlide product={product} isActive={index === activeIndex} />
          </SwiperSlide>
        ))}
      </Swiper>
      ...
      <button
        className="swiper-button-prev-custom absolute left-4 top-1/2 -translate-y-1/2 z-20  text-white p-3 rounded-full transition-colors duration-200"
        aria-label="Previous slide"
      >
        <ChevronLeft size={24} />
      </button>
      <button
        className="swiper-button-next-custom absolute right-4 top-1/2 -translate-y-1/2 z-20 text-white p-3 rounded-full transition-colors duration-200"
